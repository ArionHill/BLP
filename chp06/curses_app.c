...
char *extended_menu[] = {
    "add new CD",
    "find CD",
    "count CDs and tracks int the catalog",
...
void insert_title(char *cdtitle) {
    ...
    if (!fp) {
        mvprintw(ERROR_LINE, 0, "Cannot open CD title databse");
    } else {
        fprintf(fp, "%s\n", cdtitle);
        fclose(fp);
    }
...
void find_cd()
{
    ...
    if (titles_fp) {
        while (fgets(entry, MAX_ENTRY, titles_fp)) {
            catalog = entry;
            // mvprintw(Q_LINE + 1, 0, "found1: %s", found);
            mvprintw(Q_LINE - 1, 0, "STRSTR: %s",strstr(catalog, ","));
            if (found = strstr(catalog, ",")) {
                mvprintw(Q_LINE - 2, 0, "found2: %s", found);
                *found = '\0';                                 //
                title = found + 1;                             //
                mvprintw(Q_LINE - 3, 0, "title: %s", title);
                if (strstr(title, ",")) {             //
                    *found = '\0';                             //

                    if (strstr(title, match)) {
                        count ++;
